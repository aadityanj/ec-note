    ...
    <div className={item.isMax ? 'note fullscreen': 'note' } > 
        <div className="note-header">
            ...
            <Button.Group className="float-right">
                ...
                {  item.isMax &&
                <Button icon className="note-action" onClick={()=>handleClickEvent(item.id, index, 'min')}>
                    <Icon name="window minimize outline" />
                </Button> 
                }
                {  item.isMax &&
                <Button icon className={item.isHistory ? 'note-action active': 'note-action'}   onClick={()=> handleClickEvent(item.id, index, 'history')  }>
                    <Icon name="history" />
                </Button> 
                }
                { !item.isMax &&
                <Button icon className="note-action" onClick={()=>handleClickEvent(item.id, index, 'max')}>
                ...
                <Button icon className="note-action" onClick={()=>handleClickEvent(item.id, index, 'trash')}>
                    <Icon name='trash' />
                </Button>
            </Button.Group>
        </div>
        <div className="note-text">
                ...
                <div className="revisionlist">
                    { item.history.map( (history, hIndex) => {
                        return <div className={history.active? 'history-active': ''} key={hIndex} onClick={(e) => handleClickEvent(index, hIndex, 'selectHistory')}>
